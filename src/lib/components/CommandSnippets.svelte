<script lang="ts">
  ...
  const SNIPPETS_FILE = "C:\\Users\\Lenovo\\AppData\\Local\\Temp\\nyxedit\\snippets.json";
  ...
  async function runSnippet(snippet: Snippet, terminalId: string) {
    try {
      ...
      if (snippet.language === "python") {
        const f = `C:\\Users\\Lenovo\\AppData\\Local\\Temp\\nyxedit\\snip_${snippet.id}.py`;
        await invoke("fs_write_file", { path: f, content: cmd });
        cmd = `python "${f}"`;
      } else if (snippet.language === "node") {
        const f = `C:\\Users\\Lenovo\\AppData\\Local\\Temp\\nyxedit\\snip_${snippet.id}.js`;
        await invoke("fs_write_file", { path: f, content: cmd });
        cmd = `node "${f}"`;
      }
